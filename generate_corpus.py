...
def generate_dyck(
    length: int, max_depth: int, k: int = 2, exact_depth: int | None = None
):
    ...
    if exact_depth is not None and max_depth_reached != exact_depth:
        return None
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exact-depth", type=int, default=-1)
    ...
    exact_depth = args.exact_depth if args.exact_depth != -1 else None
    ...
    with open(args.output, "w") as f:
        for i in range(args.size):
            ...
            while result is None:
                ...
                result = generate_dyck(length, args.max_depth, args.k, exact_depth)
